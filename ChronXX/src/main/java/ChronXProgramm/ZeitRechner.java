...
//		Ueberprueft ob es Eintraege am gewuenschten Tag gibt und ersetzt diese durch die neuen Eintraege.
	...
	public boolean wurdePauseEingehalten(Arbeiter MitArbeiter, int tag)
	{
		...
		List<LocalDateTime> zeitArbeitsTag = MitArbeiter.zeit.zeitarbeitsTag.get(tag);
	...
	public double gibGleitzeitGesamt(Arbeiter mitarbeiter)
	{
		...
		{
			gleitzeit += rechneZeitarbeitsTag(mitarbeiter.zeit.zeitarbeitsTag.get(i), mitarbeiter) - (mitarbeiter.zeit.wochenstunden / 6);
		}
	...
	public double gibGleitzeitJahr(Arbeiter mitarbeiter)
	{
		...
		for(int i = 0; i < mitarbeiter.zeit.zeitarbeitsTag.size(); i++)
		{
			if(mitarbeiter.zeit.zeitarbeitsTag.get(i).get(0).getYear() == LocalDateTime.now().getYear())
			{
				gleitzeit += rechneZeitarbeitsTag(mitarbeiter.zeit.zeitarbeitsTag.get(i), mitarbeiter) - (mitarbeiter.zeit.wochenstunden / 6);
			}
		}
	...
	public double gibGleitzeitQuartal(Arbeiter mitarbeiter)
	{
		...
		{
			if(mitarbeiter.zeit.zeitarbeitsTag.get(i).get(0).getYear() == LocalDateTime.now().getYear()
			   && mitarbeiter.zeit.zeitarbeitsTag.get(i).get(0).get(IsoFields.QUARTER_OF_YEAR) == LocalDateTime.now().get(IsoFields.QUARTER_OF_YEAR))
			{
				gleitzeit += rechneZeitarbeitsTag(mitarbeiter.zeit.zeitarbeitsTag.get(i), mitarbeiter) - (mitarbeiter.zeit.wochenstunden / 6);
			}
		}
	...
	public double gibGleitzeitMonat(Arbeiter mitarbeiter)
	{
		...
		{
			if(mitarbeiter.zeit.zeitarbeitsTag.get(i).get(0).getYear() == LocalDateTime.now().getYear()
			   && mitarbeiter.zeit.zeitarbeitsTag.get(i).get(0).getMonth() == LocalDateTime.now().getMonth())
			{
				gleitzeit += rechneZeitarbeitsTag(mitarbeiter.zeit.zeitarbeitsTag.get(i), mitarbeiter) - (mitarbeiter.zeit.wochenstunden / 6);
			}
		}
	...
	private double rechneZeitarbeitsTag(List<LocalDateTime> zeitarbeitstag, Arbeiter mitarbeiter)
	{
		...
		if(zeitarbeitstag.size() == 1)
		{
			...
			{
				zeitProTag += (mitarbeiter.zeit.wochenstunden / 6);
			}
		}
