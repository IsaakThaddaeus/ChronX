...
//		Ueberprueft ob es Eintraege am gewuenschten Tag gibt und ersetzt diese durch die neuen Eintraege.
	...
	public boolean wurdePauseEingehalten(Arbeiter MitArbeiter, LocalDate tag) {
		...
		for (int i = 0; i < MitArbeiter.zeit.zeitarbeitsTag.size(); i++) {
			if (MitArbeiter.zeit.zeitarbeitsTag.get(i).get(0).toLocalDate().equals(tag)
					&& MitArbeiter.zeit.zeitarbeitsTag.get(i).size() % 2 == 0) {
	...
	public double gibGleitzeitGesamt(Arbeiter mitarbeiter) {
		...
		for (int i = 0; i < mitarbeiter.zeit.zeitarbeitsTag.size(); i++) {
			gleitzeit += rechneZeitarbeitsTag(mitarbeiter.zeit.zeitarbeitsTag.get(i), mitarbeiter)
					- (mitarbeiter.zeit.wochenstunden / 6);
		}
	...
	public double gibGleitzeitQuartal(Arbeiter mitarbeiter) {
		...
		for (int i = 0; i < mitarbeiter.zeit.zeitarbeitsTag.size(); i++) {
			if (mitarbeiter.zeit.zeitarbeitsTag.get(i).get(0).getYear() == LocalDateTime.now().getYear()
					&& mitarbeiter.zeit.zeitarbeitsTag.get(i).get(0).get(IsoFields.QUARTER_OF_YEAR) == LocalDateTime
